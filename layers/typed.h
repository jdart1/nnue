...
public:
    ...
    virtual void forward(const void * input, void * output) const noexcept {
        ...
        doForward(static_cast<const InputType *>(input),static_cast<OutputType *>(output));
    }

    virtual void doForward(const InputType *input, OutputType *output) const noexcept = 0;

    virtual size_t getInputSize() const noexcept {
        return inputSize;
    }
    
    virtual size_t getOutputSize() const noexcept {
        return outputSize;
    }
